import { gdata, websites_get_name } from "./globals.js";
...
function task_list_include(id, name, url) {
    for (let i = 0; i < id; i++) {
        if (
            gdata.task_name_list[i] === name &&
            gdata.task_url_list[i] === url
        ) {
            return true;
        }
    }
...
function unique_task_list() {
    for (let i = gdata.task_name_list.length - 1; i >= 0; i--) {
        if (
            task_list_include(
                i,
                gdata.task_name_list[i],
                gdata.task_url_list[i]
            )
        ) {
            ...
            chrome.tabs.sendMessage(
                ...
                (response) => {
                    ...
                    if (typeof response != "string" || response.trim() == "") {
                        ...
                        callback(
                            websites_get_name[i][3](response, tabs[0].url)
                        );
                    }
                }
            );
...
function add_task(pos) {
    chrome.tabs.query(
        ...
        (tabs) => {
            ...
            get_name(tabs, (name) => {
                if (
                    !task_list_include(gdata.task_name_list.length, name, url)
                ) {
                    if (pos == "head") {
...
function export_task_list() {
    ...
    a.href = url;
    a.download = `task_list${year}_${month}_${day}.json`;
...
function import_task_list_cover() {
    ...
    input.addEventListener("change", () => {
        ...
        reader.addEventListener("loadend", () => {
            ...
            if (
                !Array.isArray(task_list.task_url_list) ||
                !Array.isArray(task_list.task_url_list)
            ) {
            ...
            if (
                task_list.task_url_list.length !==
                task_list.task_url_list.length
            ) {
...
function import_task_list_insert() {
    ...
    input.addEventListener("change", () => {
        ...
        reader.addEventListener("loadend", () => {
            ...
            gdata.task_name_list.splice(
                index - 1,
                0,
                ...task_list.task_name_list
            );
            gdata.task_url_list.splice(
                index - 1,
                0,
                ...task_list.task_url_list
            );
            unique_task_list();
            chrome.storage.local.set({
                task_name_list: gdata.task_name_list,
...
document.addEventListener("DOMContentLoaded", () => {
    document
        .getElementById("add-task-head")
        .addEventListener("click", add_task_head);
    document
        .getElementById("add-task-tail")
        .addEventListener("click", add_task_tail);
    document
        .getElementById("back-to-task-list")
        .addEventListener("click", back_to_task_list);
    document
        .getElementById("submit-change-name")
        .addEventListener("click", change_name);
    document
    ...
    document
        .getElementById("submit-change-url")
        .addEventListener("click", change_url);
    document
        .getElementById("change-url")
        .addEventListener("keydown", (event) => {
            if (event.key == "Enter") {
                change_url();
            }
    ...
    document
        .getElementById("submit-change-pos")
        .addEventListener("click", submit_change_pos);
    document
        .getElementById("change-pos")
        .addEventListener("keydown", (event) => {
            if (event.key == "Enter") {
                submit_change_pos();
            }
    ...
    document
        .getElementById("export-task-list")
        .addEventListener("click", export_task_list);
    document
        .getElementById("import-task-list-cover")
        .addEventListener("click", import_task_list_cover);
    document
        .getElementById("import-task-list-insert")
        .addEventListener("click", import_task_list_insert);

    make_label_list();

    chrome.storage.local
        .get(["task_name_list", "task_url_list"])
        .then((result) => {
