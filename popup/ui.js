...
export function make_edit_task_list() {
	...
	document.getElementById("change-name").value = gdata.task_name_list[gdata.noweditid];
	document.getElementById("change-url").value = gdata.task_url_list[gdata.noweditid];
	...
	if (gdata.noweditid != 0) {
		add_edit_task(fragment, gdata.noweditid - 1, gdata.task_name_list[gdata.noweditid - 1], false);
	}
	add_edit_task(fragment, gdata.noweditid, gdata.task_name_list[gdata.noweditid], true);
...
export function make_task_list() {
	...
	for (let i = 0; i < gdata.task_name_list.length; i++) {
		...
		new_task.innerHTML = `<div class="task_number">${gdata.nowlabelid == 0 ? `${i + 1}` : `${len}<br>(${i + 1})`}</div>
		...
		new_task.querySelector("div.task-name-box > span.task-name").textContent = gdata.task_name_list[i];
...
export function make_label_list() {
	...
	for (let i = 0; i < websites.length; i++) {
		...
		function change_label() {
			document.getElementById(websites[gdata.nowlabelid][0]).className = "label";
			gdata.nowlabelid = i;
