...
export const websites_get_name = [
    ...
    [
        ...
        function (response, url) {
            url = url.trim().split("#")[0];
            return (
                "LG-training_" + url.split("/").pop() + " " + response.trim()
            );
        },
    ],
    [
    ...
    [
        ...
        function (response, url) {
            return "AT-Contest_" + response.trim();
        },
    ],
    [
    ...
    [
        /^https:\/\/qoj\.ac\/contest\/\d+$/,
        "qojContest",
        "body > div.container > div.uoj-content > div.text-center > h1",
        function (response, url) {
            url = url.trim().split("#")[0];
            return (
                "QOJ-Contest_" + url.split("/").pop() + " " + response.trim()
            );
        },
    ],
    [
        /^https:\/\/www\.cnblogs\.com\/[^\/]+\/p\/\d+(.html)?$/,
        "cnblog",
        "#sbTitleText",
        function (response, url) {
            url = url.trim().split("#")[0];
    ...
    [
        /^https:\/\/www\.cnblogs\.com\/[^\/]+\/articles\/\d+(.html)?$/,
        "cnblog",
